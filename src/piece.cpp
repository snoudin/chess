...
std::vector<std::shared_ptr<Move>> WalkingPiece::getMoves(Position& board, get_piece_func_type get_piece,
                                                          last_move_func_type) {
...
std::vector<std::shared_ptr<Move>> JumpingPiece::getMoves(Position& board, get_piece_func_type get_piece,
                                                          last_move_func_type) {
    std::vector<std::shared_ptr<Move>> return_moves;
...
std::vector<std::shared_ptr<Move>> Pawn::getMoves(Position& board, get_piece_func_type get_piece,
                                                  last_move_func_type last_move) {
    std::vector<std::shared_ptr<Move>> return_moves;
    ...
    if (!to.outOfBounds() && !(board.*get_piece)(to)) {
        ...
        if (to.get_y() == promotion_y[color]) {
            return_moves.push_back(std::shared_ptr<Move>(dynamic_cast<Move*>(
                            new PromotionMove(*mv, std::shared_ptr<Piece>(new Knight(to, color))))));
            return_moves.push_back(std::shared_ptr<Move>(dynamic_cast<Move*>(
                            new PromotionMove(*mv, std::shared_ptr<Piece>(new Bishop(to, color))))));
            return_moves.push_back(std::shared_ptr<Move>(dynamic_cast<Move*>(
                            new PromotionMove(*mv, std::shared_ptr<Piece>(new Rook(to, color))))));
            return_moves.push_back(std::shared_ptr<Move>(dynamic_cast<Move*>(
                            new PromotionMove(*mv, std::shared_ptr<Piece>(new Queen(to, color))))));
        } else {
    ...
    for (value_type i = 0; i < 2; ++i) {
        ...
        if (piece && piece->getColor() != color) {
            ...
            if (to.get_y() == promotion_y[color]) {
                ...
                return_moves.push_back(std::make_shared<Move>(mv));
            }
        } else if (last_mv
                   && last_mv->getTo() == coordinates + enpassant_target[i]
                   && last_mv->getMovedPiece()->getName() == PieceType::pawn
                   && abs(last_mv->getTo().get_y() - last_mv->getFrom().get_y()) == 2) {  // pawn did long move
            return_moves.push_back(
                    std::make_shared<Move>(shared_from_this(), coordinates, to, last_mv->getMovedPiece()));
        }
    }
    return return_moves;
}
