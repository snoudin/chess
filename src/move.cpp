...
Move::Move(std::shared_ptr<Piece> moved_piece, const geometry::Point& from, const geometry::Point& to,
           std::shared_ptr<Piece> captured):
moved_piece(std::move(moved_piece)), from(from), to(to), captured(std::move(captured)) {}

PromotionMove::PromotionMove(const Move& mv, std::shared_ptr<Piece> new_piece): Move(mv),
                             new_piece(std::move(new_piece)) {}
