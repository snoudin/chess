...
FollowingPosition* Position::makeMove(std::shared_ptr<Move> mv) {
    mv->makeMove();
    start->which_turn = Color(!bool(start->which_turn));
    ...
    if (mv->getCaptured()) start->used[mv->getCaptured()->getCoordinates().get_x()][mv->getCaptured()->getCoordinates().get_y()] = nullptr;
...
Position* FollowingPosition::rollbackMove() {
    ...
    if (mv->getCaptured()) start->used[mv->getCaptured()->getCoordinates().get_x()][mv->getCaptured()->getCoordinates().get_y()] = mv->getCaptured();
