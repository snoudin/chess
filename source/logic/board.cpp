...
bool Board::valid(std::shared_ptr<Move> mv) {
    ...
    for (auto piece: start.all_pieces) {
        if (piece->getName() == "King" && piece->getColor() != start.which_turn) king = piece;
    }
...
std::vector<std::shared_ptr<Move>> Board::get_all_moves() {
    ...
    for (auto piece: start.all_pieces) {
        if (piece->getColor() != start.which_turn || piece->getIsCaptured()) continue;
        std::vector<std::shared_ptr<Move>> new_moves = piece->getMoves(*cur_pos, &Position::get_piece, &Position::get_last_move);
...
std::vector<std::shared_ptr<Move>> Board::get_castle_moves() {
    ...
    for (auto piece: start.all_pieces) {
        if (piece->getColor() != start.which_turn || piece->getIsCaptured()) continue;
        if (piece->getName() == "King") king = piece;
        if (piece->getName() == "Rook") rooks.push_back(piece);
    }
        ...
        for (auto dir: castle_direction) {
            ...
            for (auto piece : start.all_pieces) {
                ...
                for (value_type i = 0; i <= 2; ++i) {
                    if (piece->canBeCaptured(*cur_pos, &Position::get_piece, king->getCoordinates() + dir * i)) is_free = false;
                }
            }
            if (!is_free) continue;
            castle_moves.push_back(std::shared_ptr<Move>(dynamic_cast<Move*>(
                    new CastleMove(king, rook, king->getCoordinates(), king->getCoordinates() + dir * 2, rook->getCoordinates(),
...
bool Board::dead_position() {
    ...
    for (auto piece : start.all_pieces) {
        ...
        if (piece->getName() == "Queen") return false;
        if (piece->getName() == "Rook") return false;
        if (piece->getName() == "Pawn") return false;
        if (piece->getName() == "Knight") ++cnt_of_light[piece->getColor()];
        if (piece->getName() == "Bishop") ++cnt_of_light[piece->getColor()];
    }
...
size_t Board::get_hash(size_t count_of_moves) {
    ...
    return std::hash<std::string>()(field + std::to_string(count_of_moves) + std::to_string(bool(start.which_turn)));
}
...
bool Board::fifty_moves() {
    ...
    while (count < 50 && pos != &start) {
        ...
        if (following_pos->mv->getMovedPiece() && following_pos->mv->getMovedPiece()->getName() == "Pawn") return false;
